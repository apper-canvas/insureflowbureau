...
import ClaimStatusTracker from "@/components/molecules/ClaimStatusTracker";
  ...
  return (
    ...
    <Card className="h-full">
        <div className="flex items-start justify-between mb-4">
            <div className="flex items-center space-x-3">
                ...
                <div>
                    ...
                    <p className="text-sm text-surface-500">Claim ID: {claim.id}
                    </p>
                </div>
            </div>
        ...
        <div className="space-y-3">
            ...
            {claim.status === "approved" && claim.settlementAmount && <div className="flex justify-between">
                <span className="text-sm text-surface-500">Settlement Amount:</span>
                <span className="font-semibold text-success">
                    {formatCurrency(claim.settlementAmount)}
                </span>
            </div>}
            {claim.status === "processing" && claim.estimatedSettlement && <div className="flex justify-between">
                <span className="text-sm text-surface-500">Estimated Settlement:</span>
                <span className="font-semibold text-warning">
                    {formatCurrency(claim.estimatedSettlement)}
                </span>
            </div>}
            {claim.status === "rejected" && claim.rejectionReason && <div className="mt-3 p-3 bg-error/5 rounded-lg">
                <p className="text-sm text-error">
                    <strong>Rejection Reason:</strong> {claim.rejectionReason}
                </p>
            </div>}
        </div>
        {/* Status Tracker for active claims */}
        {(claim.status === "pending" || claim.status === "processing") && <ClaimStatusTracker claim={claim} />}
    </Card>
</motion.div>
  )
}
